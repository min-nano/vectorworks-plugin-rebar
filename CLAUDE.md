...
- **平面線は Top/Plan コンポーネント（10）として明示設定する**。断面コンポーネント（6/9）のグループも PIO の 2D プロファイルに残るため、Top/Plan を明示定義しないと断面表現が平面ビューにも漏れて表示される。平面線をグループにまとめて Top/Plan に固定することで、平面ビューには平面線だけ・断面ビューポートには断面線だけが表示される。`Set2DComponentGroup` が使えない環境（VW 2018 以前）では 2D コンポーネントの仕組みが無く、平面線グループがそのまま平面ビューの 2D 表現になるため削除しない（断面線グループはこの環境では削除するので平面ビューには平面線だけが残る）。
