...
- 梁のせん断補強筋は各区間の始端から `ピッチ/2` を最初とし等ピッチ（区間長がピッチ未満なら中央に 1 本）。断面 2D コンポーネントは**区間ごとに横断面のみ**を実位置へ生成し、各区間の向き（X 軸寄り/Y 軸寄り）で target（left_right/front_back）を決める。これにより折れ線・矩形パスでも各区間を切断した位置に断面が出る（2D コンポーネントはローカル軸 6 方向にしか持てないため、斜め区間は近い軸へ寄せた近似）。**縦断面（軸方向に走る梁の立面）は生成しない**: 2D コンポーネントは切断位置に応じた条件表示ができず常に表示されるため、切断面が通らない「紙面に平行な梁」の配筋まで断面ビューポートに出てしまう。横断面だけに絞ることで、実際に切断された区間の断面だけを表示する。
