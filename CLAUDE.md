...
4. **描画（フェーズ2）** — `vw.execute_document(document, pio_handle)` が検証後、平面線 → 3D 鉄筋 → 断面 2D コンポーネントの順で描画する。
...
- **平面線はパスオブジェクトの 2D プロファイルに固定する**（`vs.SetCustomObjectProfileGroup(pio, planGroup)`）。リセットで描いた全図形が既定で PIO のプロファイル（＝デザインレイヤの平面ビューに出る 2D）になり、`Set2DComponentGroup` が成功（戻り値 True）を返しても断面線はプロファイルから取り除かれず平面ビューに漏れる（VW 上で確認済み）。プロファイルを平面線グループに固定することで、平面ビューには平面線だけ・断面ビューポートには断面線だけが表示される。断面コンポーネント割り当ての後にプロファイル固定を行う。`SetCustomObjectProfileGroup` が無い環境では何もしない。
- **2D 線は画面平面（screen plane, planar ref 0）に置く**（`vw/draw.py` の `set_screen_plane` = `vs.SetPlanarRef(handle, 0)`）。2D コンポーネントのグループは画面平面のオブジェクトでないと `Set2DComponentGroup` が各コンポーネントのコンテナへ移動できず、断面表現（前後/左右の断面）が層平面の 2D として PIO 本体に残り、断面ビューポートを作らなくてもデザインレイヤの平面ビューに漏れて表示される。`SetPlanarRef` が無い環境（VW 2018 以前）は 2D コンポーネントの仕組み自体が無いため何もしない。
- 断面線は `vs.BeginGroup`/`vs.EndGroup` でグループにまとめてから設定する（`Set2DComponentGroup` は既存グループを**置換**するため、リセットごとの残骸が残らない）。命令が無い target は NULL ハンドル（`vs.Handle(0)`）を設定して既存グループを削除する。`Set2DComponentGroup` が使えない環境（VW 2018 以前）ではグループを `vs.DelObject` で削除し、平面ビューを汚さない。
- **`Set2DComponentGroup` の戻り値は成否判定に使う**（`component_set_succeeded`）。公式 VS ラッパーは BOOLEAN（成功=True）だが、内部 SDK は `ESetSpecialGroupErrors`（NoError=0, CannotSet_BadData, CannotSet_UserSpecified）を返すため、`bool` は True を、`int` は 0（NoError）を成功として扱う。割り当てに失敗した断面グループは平面ビューに残るため `vs.DelObject` で削除する。
- **`vs.SetTopPlan2DComp(pio, 0)` で Top/Plan ビューを Top（非断面）に固定する**（0=Top, 1=Top and Bottom Cut）。断面コンポーネント（3/6/9）が平面ビューに漏れないための直接的な対策。関数が無い環境（VW 2018 以前）は何もしない。
- **断面表現が平面ビューに漏れる問題は VW 上で切り分け中**。`run()` は各 2D コンポーネント割り当ての生の戻り値（`Set2DComponentGroup` / `SetTopPlan2DComp`）を `vs.Message` でステータスバーに表示する（`_diagnostic_message`）。**問題解決後に削除する一時的な診断出力**。
...
- 梁のせん断補強筋は各区間の始端から `ピッチ/2` を最初とし等ピッチ（区間長がピッチ未満なら中央に 1 本）。断面 2D コンポーネントは**最長区間の向き**で横断面/縦断面の target を決める。
