...
homeskz の main.py と同じ仕組み（GitHub `main` ブランチのコミット SHA 比較 → アーカイブ直接展開 → 依存は pip）だが、**PIO のリセットは図形の移動・編集のたびに実行される**ため、更新確認（ネットワークアクセス）は「本体パッケージが `sys.modules` に無いとき＝VectorWorks セッション内の初回」だけ行う。毎リセットの確認は編集操作を著しく遅くするため行わない。更新は VectorWorks の再起動後に反映される。
...
- すべての図形に `vs.SetClass` でクラスを割り当て、描画属性（線色・塗色・太さ・線種・パターン・マーカー・透明度）を属性ごとの by-class 設定関数で**すべてクラス属性に従わせる**（homeskz の `_set_all_attributes_by_class` と同じ規約）。ユーザーはクラス側で上端筋・下端筋等の線種・色を調整する。クラス名は `rebar/classes.py` の定数に集約（存在しないクラスは VW が自動生成）。
